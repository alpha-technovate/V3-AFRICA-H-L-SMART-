...
import type { Patient, ChronicConditionSummary, ChronicCondition } from '@/lib/types';
...
export function ConditionsTab({ patient }: ConditionsTabProps) {
  // FIX: Use ChronicConditionSummary[] because that's what patient.chronicConditions is
  const [conditions, setConditions] = useState<ChronicConditionSummary[]>(patient.chronicConditions);
  const [open, setOpen] = useState(false);

  // FIX: New chronic condition also matches the summary form (no patientId/createdAt/updatedAt)
  const handleSaveCondition = (newCondition: Omit<ChronicConditionSummary, 'id'>) => {
                ...
                <TableRow key={condition.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className="text-sm">
                      {condition.icd10Code.toUpperCase()}
                    </Badge>
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={cn('border-0 font-normal', severityColorMap[condition.severity])}
                    >
                      {condition.severity}
                    </Badge>
                  </TableCell>
...
function AddConditionDialog({
  ...
  onSave: (condition: Omit<ChronicConditionSummary, 'id'>) => void;
  onClose: () => void;
}) {
  ...
  const handleSubmit = () => {
    ...
    onSave({
      ...formState,
      addedBy: 'Dr. Evelyn Reed', // placeholder
    });
  };
  ...
  return (
    <DialogContent className="sm:max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>Fill in the details for the new chronic condition.</DialogDescription>
      </DialogHeader>
      ...
      <div className="grid gap-4 py-4">
        ...
        <div className="grid grid-cols-4 items-center gap-4">
          <Label htmlFor="icd10Code" className="text-right">
            ICD-10
          </Label>
        ...
        <div className="grid grid-cols-4 items-center gap-4">
          ...
          <Select
            onValueChange={v => setFormState(p => ({ ...p, severity: v as any }))}
